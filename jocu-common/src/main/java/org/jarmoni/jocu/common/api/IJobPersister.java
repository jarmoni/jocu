...
public interface IJobPersister {
	...
	/**
	 * Inserts job into store. Follwing properties must be set in
	 * implementation:
	 * <ul>
	...
	/**
	 * Resumes a (paused) job. Following properties must be set in
	 * implementation:
	 * <ul>
	 * <li>jobState = <code>WAITING</code></li>
	...
	/**
	 * Updates the job-object. Following properties must be set in
	 * implementation:
	 * <ul>
	...
	/**
	 * Sets <code>state = EXCEEDED</code> if the following conditions are
	 * fulfilled:<br>
	 * <ul>
	 * <li><code>jobState = PROCESSING || jobState = COMPLETING</code>
	 * <b>and</b></li>
	 ...
	 * <li>if <code>jobState = COMPLETING</code> ->
	 * <code>jobState = FINISHED</code></li>
	 * </ul>
	 * 
	 */
	void refresh();
